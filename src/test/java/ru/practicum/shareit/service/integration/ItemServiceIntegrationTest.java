...
public class ItemServiceIntegrationTest {
    ...
    @Test
    @DisplayName("Проверка метода save для Comment. Выбрасывает ValidationException, если пользователь не использовал вещь")
    void testSave_whenUserDidNotUseItem_thenThrowValidationException() {
        ...
        when(bookingRepository.findAllByItemIdAndBookerIdAndStatusAndEndIsBefore(
                        comment.getItem().getId(), comment.getAuthor().getId(), Status.APPROVED, comment.getCreated()
                )).thenReturn(List.of(booking));
        when(commentRepository.save(any(Comment.class))).thenReturn(comment);
        Assertions.assertThrows(ValidationException.class, () -> itemService.addComment(requestComment));
        verify(userRepository, times(1)).findById(anyInt());
        verify(itemRepository, times(1)).findById(anyInt());
        verify(bookingRepository, times(1)).findAllByItemIdAndBookerIdAndStatusAndEndIsBefore(
